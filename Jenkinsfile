...
properties([
  parameters([
    ...
    string(name: 'MODULE_PATH', defaultValue: 'vm/compute', description: 'Terraform or deployment module path'),
    ...
    string(name: 'ACTION', defaultValue: 'plan', description: 'Terraform action (plan/apply/destroy/init)'),
    booleanParam(name: 'CHECKOUT_SHARED_LIBRARY', defaultValue: true, description: 'Checkout shared library into workspace?'),
    string(name: 'SHARED_LIBRARY_URL', defaultValue: 'https://github.com/yourorg/jenkins-shared-library.git', description: 'URL of the shared library repo'),
